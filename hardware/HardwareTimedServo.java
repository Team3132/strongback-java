...
/*
 * Not all servos are instantaneous.
 * This class is for servos that take a finite amount of time to move from one position to another position.
 * Because servos do not provide feedback, and not all servos can be cleanly interrupted this class provides
 * a servo that will complete a command before accepting another command.
 */
...
public class HardwareTimedServo implements Servo {
	...
	HardwareTimedServo(edu.wpi.first.wpilibj.Servo servo, double min, double max, double travelPerSecond, double initial) {
		this.servo = servo;
		this.min = min;
		this.max = max;
	...
	public Servo setTarget(double value) {
		...
		if (value != target) {
			 ...
			 */
			target = value;
			endTime = Strongback.timeSystem().currentTime() + (Math.abs(target - current) / travelPerSecond);
			servo.set(target);
		}
		return this;
	}
