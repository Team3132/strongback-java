...
public class MotorFactory {
	...
	public static Motor getDriveMotor(boolean leftMotor, Clock clock) {
		...
		switch (Config.drivebase.motorControllerType) {
			...
			if (Config.drivebase.currentLimiting) {
				...
				spark.setSmartCurrentLimit(Config.drivebase.contCurrent, Config.drivebase.contCurrent);
		...
		default:
			Log.error("Invalid drive motor controller '%s'. Please use 'TalonSRX' or 'SparkMAX'. Using TalonSRX.",
		...
		case Config.motorController.talonSRX:
			 ...
			 */
			if (Config.drivebase.currentLimiting) {
				talon.configContinuousCurrentLimit(Config.drivebase.contCurrent, 0); // limit to 35 Amps when current exceeds 40 amps for
																	// 100ms
	...
	public static HardwareTalonSRX getColourWheelMotor() {
		HardwareTalonSRX motor = getTalon("colour", Config.colourWheel.canID, true, NeutralMode.Brake, Config.colourWheel.pidf);
	...
	public static HardwareTalonSRX getShooterMotor(Clock clock) {
		HardwareTalonSRX motor = getTalon("shooter", Config.shooter.canIds, false, NeutralMode.Coast,
				Config.shooter.pidf);
	...
	private static HardwareTalonSRX getTalon(String name, int[] canIDs, boolean invert, NeutralMode mode, 
    	...
    	for (int n = 1; n < canIDs.length; n++) {
			...
			Chart.register(() -> follower.getOutputCurrent(), "Talons/%d/Current", canIDs[n]);
		}
		return leader;
	}
    ...
    private static HardwareSparkMAX getSparkMAX(String name, int[] canIDs, boolean invert, NeutralMode mode, PIDF pidf) {
	 ...
	 */
	private static HardwareSparkMAX getSparkMAX(String name, int canID, boolean invert, NeutralMode mode, PIDF pidf) {
		Log.debug("%s: " + canID, " spark max");
		int[] canIDs = new int[1];
		canIDs[0] = canID;
