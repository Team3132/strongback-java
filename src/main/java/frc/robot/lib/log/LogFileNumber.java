 ...
 * 
 * To make the log files easier to find they should to be timestamped so that the
 * run at a specific time can be found.
 * 
 ...
 * 
 * Unfortunately the code (and the logging of its output) will start well
 * before this.
 * 
 ...
 * 
 * The different log files (chart, etc) all share the same number and they
 * are all recreated with a new number when the robot is enabled so that output
 * of every enable is in a new file.
 */
