...
public class LinearServo {
	...
	public void setServo(double inputInches) {
		...
		double limitTargetPos = MathUtil.scale(newTargetInches, 0.0, SERVO_LENGTH_INCHES, -1.0, 1.0);
		
		// See how much the servo has moved
		double targetPosDelta = Math.abs(limitTargetPos - targetPos);
		if (targetPos == limitTargetPos) return;
	...
	private double calcTime(double inchesPerSecond,  double inchesDelta) { // REX: This could probably be folded into the call on line 54
		 return inchesDelta / inchesPerSecond + System.currentTimeMillis() / 1000;	// REX: Please change to Timer.getFPGATimestamp() which is of higher accuracy and already in seconds.
	}
	
	public boolean isOnTarget() {
		return System.currentTimeMillis() / 1000 >= atPosTimeSeconds;	// REX: Please change to Timer.getFPGATimestamp()
	}
