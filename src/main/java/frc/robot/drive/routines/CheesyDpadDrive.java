 ...
 */
public class CheesyDpadDrive implements DriveRoutine {
	private String name = "CheesyDpad";
	...
	public CheesyDpadDrive(DirectionalAxis dPad, ContinuousRange throttle, ContinuousRange wheel, Switch isQuickTurn,
			Log log) {
		this.dPad = dPad;
        ...
        if (quickTurn) {
            if (Math.abs(linearPower) < kQuickStopDeadband) {
                ...
                mQuickStopAccumlator = (1 - alpha) * mQuickStopAccumlator
                        + alpha * limit(turn) * kQuickStopScalar;
            }
            overPower = 1.0;
