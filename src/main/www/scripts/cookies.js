/*
 * Javascript wrapper to allow selectable data traces.
 * We save the selection in a cookie so each reload only
 * shows the current set of traces.
 *
 * We have buttons that allow easy manipulation of the traces.
 *
 * Sets variable visList.
 */
...
for (var i = 0; i < baseLabelsLen; i++) {
	visList.push(true);
	visListNames[baseLabelsTmp[i]] = true;
}
...
function addGroups(group) {
	if (presentGroup(group)) {
		return;
	}
	var i = group.lastIndexOf('/');
	if (i != -1) {
		// check the parent also
		addGroups(group.substring(0, i));
	}
	// add the prefix, as well as any parental prefix if necessary
	//console.log("Adding prefix:", group)
...
function doSetupGroup(g) {
	...
	var gsl = gs.length;
	var gs_name = gs;
	// Strip off the leading elements in the group name, just leaving the last element.
	if (gs.lastIndexOf('/') != -1) {
	...
	if (tdu_cookie != "") {
		...
		for (var i = 0; i < tdu_ignore_array.length; i++) {
			...
			if (index < menu.entries.length) {
				doClear('cb' + index);
				visList[index] = false;
				visListNames[baseLabelsTmp[index]] = false;
				ignored++;
			}
		}
	}
