...
/**
 * Changes the visiblity of a series. This does NOT redraw the graph. You must
 * call finalVisibility() after one or more calls to changeVisibility() to redraw the graph.
 *
 * @param {number} num the series index
 * @param {boolean} value true or false, identifying the visibility.
 */
Dygraph.prototype.changeVisibility = function(num, value) {
  var x = this.visibility();
  if (num < 0 || num >= x.length) {
    console.warn("invalid series number in setVisibility: " + num);
  } else {
...
/**
 * Redraws the screen after changing the visibility of a series
 * This function shoudl be called after completing a set of changeVisibility() calls.
 *
 */
Dygraph.prototype.finalVisibility = function() {
    this.predraw_();
};
...
/* Disabled by Mark Waldron due to this next block matching timestamps like
 *  2002.123 which is a valid number of seconds since the robot started.
