...
public class TestOverridableSubsystem {
    ...
    public void testNormalMode() {
        ...
        normalIntake.setTargetRPM(1);
        // Check that only the real interface got told about it.
        assertThat(real.getTargetRPM(), is(closeTo(1.0, 0.1)));
        assertThat(simulator.getTargetRPM(), is(closeTo(0, 0.1)));
        assertThat(mock.getTargetRPM(), is(closeTo(0, 0.1)));
        // Pretend to be the diag box and send through a command.
        overrideIntake.setTargetRPM(-1);
        // Check that only the real interface got told about it.
        assertThat(real.getTargetRPM(), is(closeTo(1.0, 0.1)));
        assertThat(simulator.getTargetRPM(), is(closeTo(0, 0.1)));
        assertThat(mock.getTargetRPM(), is(closeTo(-1, 0.1)));
        ...
        // Pretend to be the controller and send through a command.
        normalIntake.setTargetRPM(0.5);
        // Check that only the simulator got told about it.
        assertThat(real.getTargetRPM(), is(closeTo(1.0, 0.1)));
        assertThat(simulator.getTargetRPM(), is(closeTo(0.5, 0.1)));
        assertThat(mock.getTargetRPM(), is(closeTo(-1, 0.1)));
        // Pretend to be the diag box and send through a command.
        overrideIntake.setTargetRPM(-0.5);
        // Check that only the real interface got told about it.
        assertThat(real.getTargetRPM(), is(closeTo(-0.5, 0.1)));
        assertThat(simulator.getTargetRPM(), is(closeTo(0.5, 0.1)));
        assertThat(mock.getTargetRPM(), is(closeTo(-1, 0.1)));
        ...
        // Pretend to be the controller and send through a command.
        normalIntake.setTargetRPM(0.25);
        // Check that only the simulator got told about it.
        assertThat(real.getTargetRPM(), is(closeTo(-0.5, 0.1)));
        assertThat(simulator.getTargetRPM(), is(closeTo(0.25, 0.1)));
        assertThat(mock.getTargetRPM(), is(closeTo(-1, 0.1)));
        // Pretend to be the diag box and send through a command.
        overrideIntake.setTargetRPM(-0.25);
        // Check that only the mock got told about it.
        assertThat(real.getTargetRPM(), is(closeTo(-0.5, 0.1)));
        assertThat(simulator.getTargetRPM(), is(closeTo(0.25, 0.1)));
        assertThat(mock.getTargetRPM(), is(closeTo(-0.25, 0.1)));
    }
