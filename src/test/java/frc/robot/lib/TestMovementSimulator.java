...
public class TestMovementSimulator {
	...
	public void calculateSpeed() {
		...
		p.idealSpeed = 5;
		assertThat(calculateSpeed(p), is(closeTo(5.0, 0.1)));
		// Set an ideal speed that requires max acceleration.
		p.idealSpeed = 10;
		assertThat(calculateSpeed(p), is(closeTo(10.0, 0.1)));
		...
		p.currSpeed = 50;
		p.idealSpeed = 55;
		assertThat(calculateSpeed(p), is(closeTo(55.0, 0.1)));
		// Set an ideal speed that requires max acceleration.
		p.idealSpeed = 60;
		assertThat(calculateSpeed(p), is(closeTo(60.0, 0.1)));
		// Set an ideal speed that requires max negative acceleration.
		p.idealSpeed = 40;
		assertThat(calculateSpeed(p), is(closeTo(40.0, 0.1)));
		// Set an ideal speed that requires more than max acceleration.
		p.idealSpeed = 65;
		assertThat(calculateSpeed(p), is(closeTo(60.0, 0.1)));
		// Set an ideal speed that requires more than max neg acceleration.
		...
		assertThat(calculateSpeed(p), is(closeTo(-5.0, 0.1)));
		p.currSpeed = -5;
		assertThat(calculateSpeed(p), is(closeTo(-15.0, 0.1)));
	...
	public void calculateIdealSpeedAtPos() {
		...
		// All zeros, no movement.
		assertThat(calculateIdealSpeed(p), is(equalTo(0.0)));
		...
		assertThat(calculateIdealSpeed(p), is(closeTo(p.maxSpeed, 0.1)));
		...
		p.targetX = 5;
		assertThat(calculateIdealSpeed(p), is(closeTo(5.0 * Math.sqrt(2), 0.1)));
		// Target is 10 inches away. d=0.5at^2 => t=sqrt(2d/a) = sqrt(2*10/5) = sqrt(4) = 2
		// v = a*t = 5 * 2 = 10
		p.targetX = 10;
		assertThat(calculateIdealSpeed(p), is(closeTo(10.0, 0.1)));
	...
	public void positionCalculatorZero() {
		...
		assertThat(calc.getTargetPos(), is(equalTo(0.0)));
		assertThat(calc.getPos(), is(equalTo(0.0)));
		assertThat(calc.getSpeed(), is(equalTo(0.0)));
	}
	...
	public void positionCalculatorRampUp() {
		...
		//     = 1/2*5*1
		//     = 2.5
		assertThat(calc.getPos(), is(closeTo(2.5, 0.1)));
		// Speed will be:
		//   v = at
		//     = 5*1 = 5
		assertThat(calc.getSpeed(), is(closeTo(5, 0.1)));
		// Add another second, should be 10 inches in.
		calc.step(1); 
		//  d = 1/2at^2
		//    = 1/2*5*4
		//    = 10
		assertThat(calc.getPos(), is(closeTo(10, 0.1)));
		// Speed will be:
		//   v = at
		//     = 5*2 = 10
		assertThat(calc.getSpeed(), is(closeTo(10, 0.1)));
		// Solve for max speed
		//   v = at
		//   t = v/a
		//     = 20/5
		...
		//    = 40
		double pos = 40;
		assertThat(calc.getPos(), is(closeTo(pos, 0.1)));
		// And the speed should be the max of 20.
		assertThat(calc.getSpeed(), is(closeTo(20, 0.1)));
	...
	public void positionCalculatorRampDown() {
		...
		calc.step(1);
		pos += 1 * (20 + 15) / 2.0;
		assertThat(calc.getSpeed(), is(closeTo(15, 0.1)));
		assertThat(calc.getPos(), is(closeTo(pos, 0.1)));
		...
		calc.step(1);
		pos += 1 * (5 + 0) / 2.0;
		assertThat(calc.getSpeed(), is(closeTo(0, 0.1)));
		assertThat(calc.getPos(), is(closeTo(pos, 0.1)));

		assertThat(calc.getPos(), is(closeTo(calc.getTargetPos(), 0.1)));
	}
