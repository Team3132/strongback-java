 ...
 * An actuator takes some time to move to its final position. How much depends on the implementation.
 * An actuator may, or may not, be able to update its target whilst it is moving.
 * 	To check if a target has been updated check with if (actuator.setTarget(target).getTarget() == target)
